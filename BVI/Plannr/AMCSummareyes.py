...
def build_stock_dictionary(df_ipis, df_stock):
...
def process_single_scenario(filepath, scenario_name, status_callback=None, scenario_num=1, total_scenarios=1, sorting_strategy=None):
    ...
    stock = build_stock_dictionary(df_ipis, df_stock)

    # Build committed_components (unchanged)
    ...
    if DEBUG_MODE:
        ...
        print(f"DEBUG: Stock for MMSU1425A = {stock.get('MMSU1425A', 'NOT FOUND')}")
        print(f"DEBUG: Committed for MMSU1425A = {committed_components.get('MMSU1425A', 'NOT FOUND')}")
        if 'MMSU1425A' in stock and 'MMSU1425A' in committed_components:
            initial_stock = stock.get('MMSU1425A', 0) + committed_components.get('MMSU1425A', 0)
            print(f"DEBUG: Initial stock before commitments = {initial_stock}")
            print(f"DEBUG: After subtracting commitments = {stock.get('MMSU1425A', 0)}")
    ...
    for _, row in df_main.iterrows():
        ...
        # Check if this is a piggyback order - UPDATED FOR PLANNED DEMAND
        try:
        ...
        # Get planned demand for this SO - UPDATED FROM BOM LOGIC
        try:
        ...
        # Calculate labor hours for this order (unchanged)
        ...
        if len(bom) > 0:
            ...
            for _, comp in bom.iterrows():
                try:
                    ...
                    if DEBUG_MODE and (comp_part == "MMSU1425A" or so == "9678487"):
                    ...
                    if available < required_qty:
                        ...
                        if DEBUG_MODE and comp_part == "MMSU1425A":
    ...
    # Calculate Kit and Instrument metrics
    releasable_results = df_results[df_results['Status'] == '✅ Release']
    
    # Kit metrics (Planner codes 3001, 3801, 5001)
    kit_planners = ['3001', '3801', '5001']
    releasable_kits = releasable_results[releasable_results['Planner'].isin(kit_planners)]
    releasable_kits_count = len(releasable_kits)
    releasable_kits_hours = releasable_kits['Hours'].sum()
    releasable_kits_qty = releasable_kits['Demand'].sum()
    
    # Instrument metrics (Planner codes 3802, 3803, 3804, 3805)
    instrument_planners = ['3802', '3803', '3804', '3805']
    releasable_instruments = releasable_results[releasable_results['Planner'].isin(instrument_planners)]
    releasable_instruments_count = len(releasable_instruments)
    releasable_instruments_hours = releasable_instruments['Hours'].sum()
    releasable_instruments_qty = releasable_instruments['Demand'].sum()
                ...
                if len(scenarios_for_comparison) > 1:
                    ...
                    for scenario in scenarios_for_comparison:
                        ...
                        comparison_data.append({
                            ...
                            'Releasable Kits': metrics['releasable_kits_count'],
                            'Kit Hours': f"{metrics['releasable_kits_hours']:,.1f}",
                            'Kit Qty': f"{metrics['releasable_kits_qty']:,}",
                            'Releasable Instruments': metrics['releasable_instruments_count'],
                            'Instrument Hours': f"{metrics['releasable_instruments_hours']:,.1f}",
                            'Instrument Qty': f"{metrics['releasable_instruments_qty']:,}"
                        })
...
"""
   ...
   🏆 BEST STRATEGY (Orders): {best_orders['sorting_strategy']}
      ...
      → Kits: {best_orders['metrics']['releasable_kits_count']:,} orders, {best_orders['metrics']['releasable_kits_hours']:,.0f} hrs, {best_orders['metrics']['releasable_kits_qty']:,} qty
      → Instruments: {best_orders['metrics']['releasable_instruments_count']:,} orders, {best_orders['metrics']['releasable_instruments_hours']:,.0f} hrs, {best_orders['metrics']['releasable_instruments_qty']:,} qty
...
🏆 BEST PERFORMER: {os.path.basename(best_scenario['filepath'])}
   ...
   🔧 Kits: {best_scenario['metrics']['releasable_kits_count']:,} orders, {best_scenario['metrics']['releasable_kits_hours']:,.0f} hrs, {best_scenario['metrics']['releasable_kits_qty']:,} qty
   🔬 Instruments: {best_scenario['metrics']['releasable_instruments_count']:,} orders, {best_scenario['metrics']['releasable_instruments_hours']:,.0f} hrs, {best_scenario['metrics']['releasable_instruments_qty']:,} qty
...
🔧 RELEASABLE KITS:
   Orders: {metrics['releasable_kits_count']:,}
   Hours: {metrics['releasable_kits_hours']:,.1f}
   Quantity: {metrics['releasable_kits_qty']:,}

🔬 RELEASABLE INSTRUMENTS:
   Orders: {metrics['releasable_instruments_count']:,}
   Hours: {metrics['releasable_instruments_hours']:,.1f}
   Quantity: {metrics['releasable_instruments_qty']:,}
...
# Title
title_label = ttk.Label(main_frame, text=f"PlanSnap", 
                       font=('Arial', 16, 'bold'))
...
no_export_frame.grid(row=3, column=0, pady=(0, 20))

no_export_var = tk.BooleanVar()
no_export_checkbox = ttk.Checkbutton(
    no_export_frame, 
    text="⚡ Quick Analysis Mode (show results only, no Excel export)",
...
status_label.grid(row=5, column=0, pady=(0, 10), sticky=tk.W)

# Results area
results_frame = ttk.LabelFrame(main_frame, text="📊 Results", padding="10")
results_frame.grid(row=6, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(10, 0))
...
copy_btn.grid(row=7, column=0, pady=(10, 10))
