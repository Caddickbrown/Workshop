    ...
    try:
        ...
        for _, row in df_main.iterrows():
            ...
            if len(bom) > 0:
                ...
                for _, comp in bom.iterrows():
                    try:
                        ...
                        if DEBUG_MODE:
                            initial_debug = stock_debug_info.get(comp_part, {"initial": "?", "committed": "?"})
                            initial_stock = initial_debug["initial"]
                            committed_demand = initial_debug["committed"]

                        if DEBUG_MODE:
                            debug_info = (f"{comp_part}: start={available}, "
                                        f"initial={initial_stock}, committed={committed_demand}, "
                                        f"need={required_qty}")
                        ...
                        if available >= required_qty:
                            ...
                            if DEBUG_MODE:
                                debug_info += f", used={required_qty}, remaining={stock[comp_part]}"
            ...
            results.append({
                ...
                "Debug Components": "; ".join(debug_component_logs) if DEBUG_MODE and debug_component_logs else "-"
