...
def load_and_process_file():
    ...
    try:
        ...
        for _, row in df_main.iterrows():
            processed += 1
            if processed % 100 == 0:
                status_var.set(f"⚙️ Processing order {processed}/{total}...")
                root.update()
        ...
        output_file = os.path.join(output_dir, f"Material_Release_Plan_{timestamp}.xlsx")
...
🎯 KEY FEATURES:
   ...
   ✓ Excel output with summary sheet"""
        ...
        messagebox.showinfo("Success!", 
                          f"Material release plan complete!\n\n"
...
# Create GUI
root = tk.Tk()
root.title("🏭 Material Release Planner (Hybrid Version)")
...
# Title
title_label = ttk.Label(main_frame, text="🏭 Material Release Planning Tool", 
                       font=('Arial', 16, 'bold'))
